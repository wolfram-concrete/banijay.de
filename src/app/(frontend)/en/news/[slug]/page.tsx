...
const LABEL = { fontFamily: SHARP, fontSize: "0.9vw", lineHeight: "100%", fontWeight: 700, letterSpacing: "0.052vw", textTransform: "uppercase" } as const;

export function generateStaticParams() {
  return NEWS_EN.map((item) => ({ slug: item.slug }));
}
...
export default async function NewsDetailPageEn({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const item = getEnglishNewsBySlug(slug);
  if (!item) notFound();
  ...
  return (
    <div style={{ background: "transparent" }}>
      ...
      <section style={{ paddingTop: "2.22vw", paddingBottom: "5.56vw" }}>
        <div className="max-[767px]:!px-[3vw]" style={{ paddingLeft: "2vw", paddingRight: "2vw" }}>
          <div className="grid max-[767px]:!grid-cols-1 max-[767px]:!gap-[8vw]" style={{ gridTemplateColumns: "5fr 7fr", columnGap: "9.17vw", rowGap: "3vw" }}>
            <div className="flex flex-col items-start self-start max-[767px]:!static max-[767px]:!flex-row max-[767px]:!flex-wrap max-[767px]:!gap-[6vw] md:sticky" style={{ gap: "2.22vw", top: "7rem" }}>
              {[
                { key: "Category", value: item.category },
                { key: "Published", value: item.date },
                { key: "Author", value: item.author },
              ].map((row) => (
                <div key={row.key} className="flex flex-col items-start" style={{ gap: "1.11vw" }}>
                  <div style={{ ...LABEL, color: MEDIUM }} className="max-[767px]:!text-[2.6vw]">{row.key}</div>
      ...
      <div style={{ paddingLeft: "2vw", paddingRight: "2vw" }}><div style={{ height: "1px", background: "rgba(248,247,243,0.16)" }} /></div>

      <section style={{ paddingTop: "5.56vw", paddingBottom: "8.33vw" }}>
        <div className="max-[767px]:!px-[3vw]" style={{ paddingLeft: "2vw", paddingRight: "2vw" }}>
          <h2 className="m-0 uppercase max-[767px]:!text-[9vw]" style={{ fontFamily: SHARP, fontSize: "4.44vw", fontWeight: 500, letterSpacing: "-0.139vw", marginBottom: "2.22vw", color: INK }}>More news</h2>
          <div className="grid grid-cols-3 max-[767px]:!grid-cols-1" style={{ gap: "1vw" }}>
            {more.map((entry) => (
              <Link key={entry.slug} href={`/en/news/${entry.slug}`} className="group relative flex items-end overflow-clip no-underline max-[767px]:!p-[6vw]" style={{ padding: "44% 2.22vw 2.22vw", color: PAPER }}>
                <img src={entry.img} alt={entry.title} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.04]" />
                <div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.45)" }} />
                <div className="relative flex flex-col items-start" style={{ gap: "1.11vw", zIndex: 3, padding: "1.67vw", background: "rgba(0,0,0,0.28)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", boxShadow: "inset 0 0 24px 0 rgba(248,247,243,0.08), inset 0 6px 12px 0 rgba(248,247,243,0.16)" }}>
                  <span style={LABEL} className="max-[767px]:!text-[2.9vw]">{entry.category}</span>
                  <div className="max-[767px]:!text-[5.5vw]" style={{ fontFamily: SHARP, fontSize: "2vw", lineHeight: "112%", fontWeight: 500 }}>{entry.title}</div>
                  <span className="inline-flex items-center max-[767px]:!text-[3.4vw]" style={{ fontFamily: SHARP, fontSize: "1.05vw", gap: "0.4vw" }}>Read article <ArrowUpRight size={16} /></span>
                </div>
              </Link>
            ))}
          </div>
