...
const RUBRIKEN = ["Alle", "Presse", "Primetime", "Podcast", "Social"] as const;
...
export function NewsFilter({ items }: { items: FeedItem[] }) {
  ...
  const counts = useMemo(() => {
    const map: Record<string, number> = { Alle: items.length, Presse: 0, Primetime: 0, Podcast: 0, Social: 0 };
      ...
      <div className="mb-10 flex flex-wrap gap-2.5 md:gap-3">
        {RUBRIKEN.map((r) => {
          ...
          return (
            ...
            >
              {r}
              <span className={active ? "text-white/70" : "text-[rgba(248,247,243,0.4)]"}>{counts[r]}</span>
            </button>
          );
        })}
      </div>
